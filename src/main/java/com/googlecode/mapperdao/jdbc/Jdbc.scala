...
class Jdbc private (val dataSource: DataSource, val typeManager: TypeManager) {
	...
	def query(sql: String, mapper: java.util.Map[String, _] => Boolean, args: Array[_], maxResults: Int = Int.MaxValue) {
		...
		if (args.length == 0) {
			j.getJdbcOperations.query(sql, rse)
		} else {
			val rargs = args.map(typeManager.reverseConvert _)
			j.getJdbcOperations.query(sql, rse, rargs.asInstanceOf[Array[AnyRef]]: _*)
		}
	}
	...
	def update(sql: String, args: Any*): UpdateResult =
		{
			...
			if (args.length == 0) {
				...
				val a = args.toArray
				for (i <- 0 to a.length - 1) a(i) = typeManager.reverseConvert(a(i))
				val rowsAffected = j.update(sql, a)
				new UpdateResult(rowsAffected)
			}
		}
	...
	def updateGetAutoGenerated(sql: String, args: Any*): UpdateResultWithGeneratedKeys =
		{
			...
			val rowsAffected = j.getJdbcOperations.update(new PreparedStatementCreator {
				override def createPreparedStatement(con: Connection): PreparedStatement =
					{
						...
						var idx = 1
						args.foreach { arg =>
							StatementCreatorUtils.setParameterValue(ps, idx, SqlTypeValue.TYPE_UNKNOWN, typeManager.reverseConvert(arg))
							idx += 1
						}
						ps
					}
