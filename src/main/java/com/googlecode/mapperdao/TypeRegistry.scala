...
final class TypeRegistry(entities: List[Entity[_, _]]) {
...

	{
		...
		entities.foreach { entity =>
			def create[PC, T]: Type[PC, T] = Type(entity.clz.asInstanceOf[Class[T]], entity.constructor.asInstanceOf[(ValuesMap) => T with PC with Persisted], Table[PC, T](entity.table, entity.columns.reverse.asInstanceOf[List[ColumnInfoBase[T, _]]], entity.persistedColumns.asInstanceOf[List[ColumnInfoBase[T with PC, _]]]))
