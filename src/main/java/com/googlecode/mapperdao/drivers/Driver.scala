	...
	def doUpdate[PC, T](tpe: Type[PC, T], args: List[(ColumnBase, Any)], pkArgs: List[(ColumnBase, Any)]): UpdateResultWithGeneratedKeys =
		{
			...
			// find auto-generated columns
			val agColumns = tpe.table.autoGeneratedColumns.map(_.columnName).toArray
			if (agColumns.isEmpty) {
				val ur = jdbc.update(sql, args.map(_._2) ::: pkArgs.map(_._2))
				new UpdateResultWithGeneratedKeys(ur.rowsAffected, Map())
			} else {
				jdbc.updateGetAutoGenerated(sql, agColumns, args.map(_._2) ::: pkArgs.map(_._2))
			}
		}
	/**
	 * default impl of the insert statement generation
