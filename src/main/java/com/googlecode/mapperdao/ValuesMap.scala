	...
	protected[mapperdao] def fromEntity[PC, T](typeManager: TypeManager, tpe: Type[PC, T], o: T): ValuesMap =
		{
			...
			nm ++= table.toColumnAliasAndValueMap(table.columnsWithoutAutoGenerated, o).map(e => (e._1, typeManager.deepClone(e._2)))

			o match {
				case p: T with Persisted with PC =>
					// include any auto-generated columns
					nm ++= table.toPCColumnAliasAndValueMap(table.simpleTypeAutoGeneratedColumns, p).map(e => (e._1, typeManager.deepClone(e._2)))
				case _ =>
			}
