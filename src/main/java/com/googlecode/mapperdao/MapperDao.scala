...
final class MapperDao(val driver: Driver) {
	...
	private[mapperdao] def updateInner[PC, T](entity: Entity[PC, T], o: T, oldValuesMap: ValuesMap, newValuesMap: ValuesMap, entityMap: UpdateEntityMap): T with PC with Persisted =
		{
			val tpe = typeRegistry.typeOf(entity)
			...
			o match {
				case p: Persisted =>
					if (p.mock) {
						tpe.table.simpleTypeNotAutoGeneratedColumns.foreach { c =>
							if (changed(c)) {
								throw new IllegalStateException(
									"please don't update mock objects.\nMock object updated : %s.\nColumn changed: %s.\nOld value: %s.\nNew Value : %s."
										.format(o, c, oldValuesMap(c.alias), newValuesMap(c.alias))
								)
							}
						}
						return o.asInstanceOf[T with PC with Persisted]
					}
				case _ =>
			}
			// if a mock exists in the entity map or already persisted, then return
