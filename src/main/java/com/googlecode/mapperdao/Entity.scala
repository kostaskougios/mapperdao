...
abstract class ExternalEntity[ID1TYPE, ID2TYPE, T](table: String, clz: Class[T]) extends Entity[AnyRef, T](table, clz) {
	...
	def selectOneToOneReverse(selectConfig: SelectConfig, foreignIds: (ID1TYPE, ID2TYPE)): T = selectOneToOneReverse(foreignIds)
	def selectOneToOneReverse(foreignIds: (ID1TYPE, ID2TYPE)): T = throw new RuntimeException("please implement this method in your External Entities to map one-to-one-reverse externals")
