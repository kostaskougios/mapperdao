...
trait MapperDao {
	 ...
	 */
	def insert[ID, T](entity: Entity[ID, T], o: T): T with entity.PC = insert(defaultUpdateConfig, entity, o)
	 ...
	 * @see		#insert(entity,o)
	 */
	def insert[ID, T](updateConfig: UpdateConfig, entity: Entity[ID, T], o: T): T with entity.PC = {
	...
	def insert[ID, T](
		...
		entity: Entity[ID, T],
		os: List[T]
	): List[T with entity.PC] = insert0(updateConfig, entity, os).asInstanceOf[List[T with entity.PC]]
	 ...
	 */
	def update[ID, T](entity: Entity[ID, T], o: T with DeclaredIds[ID]): T with entity.PC = update(defaultUpdateConfig, entity, o)
	 ...
	 * @see		#update(entity,o)
	 */
	def update[ID, T](updateConfig: UpdateConfig, entity: Entity[ID, T], o: T with DeclaredIds[ID]): T with entity.PC = {
		if (o == null) throw new NullPointerException("o can't be null")
	...
	): List[T with entity.PC] = updateMutable0(updateConfig, entity, os).asInstanceOf[List[T with entity.PC]]
	 ...
	 */
	def update[ID, T](entity: Entity[ID, T], o: T with DeclaredIds[ID], newO: T): T with entity.PC =
	...
	): List[T with entity.PC] = updateImmutable0(updateConfig, entity, os).asInstanceOf[List[T with entity.PC]]
	...
	def merge[ID, T](
		...
		o: T,
		id: ID
	): T with entity.PC = merge(defaultSelectConfig, defaultUpdateConfig, entity, o, id)
	...
	def merge[ID, T](
		...
		o: T,
		id: ID
	): T with entity.PC = merge0(selectConfig, updateConfig, entity, o, id).asInstanceOf[T with entity.PC]
	 ...
	 *                    exists.
	 */
	def select[ID, T](entity: Entity[ID, T], id: ID): Option[T with entity.PC] = select(defaultSelectConfig, entity, id)
	 ...
	 */
	def select[ID, T](selectConfig: SelectConfig, entity: Entity[ID, T], id: ID): Option[T with entity.PC] =
		select0(selectConfig, entity, id).asInstanceOf[Option[T with entity.PC]]
