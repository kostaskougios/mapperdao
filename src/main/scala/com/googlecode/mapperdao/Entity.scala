...
abstract class Entity[ID, T](protected[mapperdao] val table: String, protected[mapperdao] val clz: Class[T]) {
	...
	protected class PKBuilder(columnName: String) {
		...
		def autogenerated[V](columnToValue: T with PC => V)(implicit m: Manifest[V]): ColumnInfo[T with DeclaredIds[ID], V] = {
			val tpe = m.erasure.asInstanceOf[Class[V]]
			var ci = ColumnInfo(PK(columnName, true, seq, m.erasure), columnToValue, tpe).asInstanceOf[ColumnInfo[T with DeclaredIds[ID], V]]
			persistedColumns ::= ci
			ci
		}
	}
