...
abstract class EntityRelationshipVisitor[R](
	...
	private def isLoaded(vmo: Option[ValuesMap], ci: ColumnInfoRelationshipBase[_, _, _, _, _]) =
		vmo.map(visitLazyLoaded || _.isLoaded(ci)).getOrElse(visitUnlinked)

	def visit(entity: Entity[_, _, _], o: Any): R = visit(entity, o, 1)
	def visit(entity: Entity[_, _, _], o: Any, currDepth: Int): R = {
		val r = m.get(o)
		val result = if (r == null && currDepth < maxDepth) {
			...
			val collected = if (o != null) entity.tpe.table.columnInfosPlain.collect {
				case ci: ColumnInfoTraversableManyToMany[Any, _, _, _] if (isLoaded(vmo, ci)) =>
				...
				case ci: ColumnInfoManyToOne[Any, _, _, _] if (isLoaded(vmo, ci)) =>
					val fo = ci.columnToValue(o)
					manyToOne(ci, fo)
					(ci, visit(ci.column.foreign.entity, fo, currDepth + 1))
				case ci: ColumnInfoOneToOne[Any, _, _, _] if (isLoaded(vmo, ci)) =>
					(ci, oneToOne(ci, ci.columnToValue(o)))
				case ci: ColumnInfoOneToOneReverse[Any, _, _, _] if (isLoaded(vmo, ci)) =>
	...
	def createR(collected: List[(ColumnInfoBase[Any, _], Any)], entity: Entity[_, _, _], o: Any): R = { null.asInstanceOf[R] }
}
