...
protected final class MapperDaoImpl(
	...
	private[mapperdao] def insertInner[ID, T](
		...
		entityMap.get[DeclaredIds[ID], T](node.newVM.identity).getOrElse {
			...
			val modified = newVM.toMutableMap
			...
			val newE = tpe.constructor(updateConfig.data, ValuesMap.fromMap(newVM.identity, finalMods))
			// re-put the actual
			entityMap.put(newVM.identity, newE)
			newE
		}
