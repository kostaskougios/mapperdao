...
class CmdPhase(typeManager: TypeManager)
{
	...
	private var alreadyProcessed = new util.IdentityHashMap[ValuesMap, List[PersistCmd]]()
	...
	private def insert[ID, T](
		...
		alreadyProcessed.get(newVM) match {
			case null =>
				val table = tpe.table
	...
	private def update[ID, T](
		...
		if (oldVM.mock) {
			...
			if (op != null) {
				...
				// make sure we don't process it again due to deep tree's
				alreadyProcessed.put(newVM -> Nil)
	...
	private val vms = scala.collection.mutable.HashMap.empty[Any, ValuesMap]
