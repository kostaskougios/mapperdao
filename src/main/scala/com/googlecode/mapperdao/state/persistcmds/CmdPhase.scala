...
class CmdPhase(typeManager: TypeManager) {
	...
	def toInsertCmd[ID, T](
		...
		newVM: ValuesMap) = insert(entity.asInstanceOf[Entity[ID, DeclaredIds[ID], T]], newVM, 0)
	def toUpdateCmd[ID, T](
		entity: Entity[ID, _ <: DeclaredIds[ID], T],
		oldValuesMap: ValuesMap,
		newValuesMap: ValuesMap) = update(entity.asInstanceOf[Entity[ID, DeclaredIds[ID], T]], oldValuesMap, newValuesMap, 0)

	private def insert[ID, T](
		entity: Entity[ID, DeclaredIds[ID], T],
		newVM: ValuesMap,
		phase: Int): PersistCmd[ID, T] = {
		alreadyProcessed.get(newVM.identity) match {
			case None =>
				...
				val rel = related(entity, None, newVM, phase)
				val op = InsertCmd(entity, newVM, columnAndValues, rel, phase)
	...
	private def update[ID, T](
		...
		if (op.isDefined) {
			...
			val op = UpdateCmd(entity, oldVM, newVM, changedColumnAndValues, Nil, phase)
			alreadyProcessed += (newVM.identity -> op)
			op
		}
	}
			...
			case ColumnInfoTraversableManyToMany(column, columnToValue, getterMethod) =>
				...
				if (oldVM.isDefined) {
					...
					newVM.valueOf[Iterable[_]](column).map {
						case p: Persisted =>
							update(foreignEntity, null, p.mapperDaoValuesMap, phase)
						case o =>
							val vm = ValuesMap.fromEntity(typeManager, foreignEntity, o)
							insert(foreignEntity, vm, phase + 1)
					}
				}
		}.flatten
	}
