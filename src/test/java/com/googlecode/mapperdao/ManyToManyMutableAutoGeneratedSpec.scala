...
object ManyToManyMutableAutoGeneratedSpec {
	...
	object ProductEntity extends Entity[IntId, Product](classOf[Product]) {
		...
		def constructor(implicit m: ValuesMap) = new Product(name, m.mutableHashSet(attributes)) with IntId with Persisted {
	...
	object AttributeEntity extends Entity[IntId, Attribute](classOf[Attribute]) {
		...
		def constructor(implicit m: ValuesMap) = new Attribute(name, value, m.mutableHashSet(products)) with Persisted with IntId {
