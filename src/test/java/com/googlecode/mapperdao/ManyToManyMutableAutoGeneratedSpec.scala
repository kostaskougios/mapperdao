...
class ManyToManyMutableAutoGeneratedSuite extends FunSuite with ShouldMatchers {
	...
	test("insert tree of entities (cyclic references) with persisted leaf entities") {
...

		{
			...
			selected.products.toSet should be === Set(inserted, inserted2)
		}
...
object ManyToManyMutableAutoGeneratedSpec {
	...
	object ProductEntity extends Entity[IntId, Product](classOf[Product]) {
		...
		def constructor(implicit m) = new Product(name, m.mutableHashSet(attributes)) with IntId with Persisted {
	...
	object AttributeEntity extends Entity[IntId, Attribute](classOf[Attribute]) {
		...
		def constructor(implicit m) = new Attribute(name, value, m.mutableHashSet(products)) with Persisted with IntId {
