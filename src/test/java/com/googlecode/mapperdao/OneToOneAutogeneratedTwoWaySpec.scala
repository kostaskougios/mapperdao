...
class OneToOneAutogeneratedTwoWaySpec extends SpecificationWithJUnit {
	...
	"select, skip one-to-one" in {
		...
		val selected = mapperDao.select(SelectConfig(skipOneToOne = Set(InventoryEntity.product)), ProductEntity, List(inserted.id)).get
		selected must_== Product(Inventory(null, 5))
	}
