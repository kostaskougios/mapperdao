...
object OneToManyAutoGeneratedSpec {
	...
	object PersonEntity extends Entity[IntId, Person]("Person", classOf[Person]) {
		...
		val houses = oneToMany(classOf[House], "person_id", _.owns)
		...
		val constructor = (m: ValuesMap) => new Person(m(name), m(surname), m(houses).toSet, m(age), m(jobPositions).toList.sortWith(_.name < _.name)) with Persisted with IntId {
			val valuesMap = m
