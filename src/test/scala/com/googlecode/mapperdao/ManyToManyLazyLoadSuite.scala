...
class ManyToManyLazyLoadSuite extends FunSuite with Matchers
{
	...
	if (Setup.database == "h2") {
		...
		test("free lazy loaded") {
			...
			val selected = mapperDao.select(SelectConfig(lazyLoad = LazyLoad.all), ProductEntity, 2).get
			Helpers.unlinkLazyLoadMemoryData(ProductEntity, selected)
			// use reflection to detect that the field wasn't set
			val r1: Set[Attribute] = reflectionManager.get("attributes", selected)
			r1 should be(Set())
			val r2: Set[Attribute] = reflectionManager.get("properties", selected)
	...
	object ProductEntity extends Entity[Int, NaturalIntId, Product]
	{
		...
		def constructor(implicit m: ValuesMap) = new Product(id, name, attributes, properties) with Stored
	}
	...
	object PropertyEntity extends Entity[Int, NaturalIntId, Property]
	{
		...
		def constructor(implicit m: ValuesMap) = new Property(id, name, value) with Stored
	}
