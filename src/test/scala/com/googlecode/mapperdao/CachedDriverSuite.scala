...
class CachedDriverSuite extends FunSuite with ShouldMatchers {
	...
	class DummyDriver extends Driver {
		...
		override def doSelect[ID, PC <: DeclaredIds[ID], T](
			selectConfig: SelectConfig,
			tpe: Type[ID, PC, T],
			where: List[(SimpleColumn, Any)]): List[DatabaseValues] = Nil
		...
		override def doSelectManyToMany[ID, PC <: DeclaredIds[ID], T, FID, FPC <: DeclaredIds[FID], F](
			...
			leftKeyValues: List[(SimpleColumn, Any)]): List[DatabaseValues] = Nil

		override def queryForList[ID, PC <: DeclaredIds[ID], T](
			queryConfig: QueryConfig,
			tpe: Type[ID, PC, T],
			sql: String,
			args: List[Any]): List[DatabaseValues] = Nil
		override def queryForLong(queryConfig: QueryConfig, sql: String, args: List[Any]): Long = -1

		override def doUpdate[ID, PC <: DeclaredIds[ID], T](
			tpe: Type[ID, PC, T],
			args: List[(SimpleColumn, Any)], pkArgs: List[(SimpleColumn, Any)]): UpdateResult =
			UpdateResult(1)
	}
