...
class JdbcSuite extends FunSuite
{
	...
	val driver = {
		...
		Database.PostgreSql.driver(jdbc, TypeRegistry(typeManager), typeManager, None)
	}
	...
	test("test update with generated keys") {
		...
		val dt3 = (now.plusSeconds(1)).withMillisOfSecond(0)
		f(jdbc.updateGetAutoGenerated("insert into test_generatedkeys(name,dt) values(?,?)", Array(idColumn), "scala", dt3)) should be(3)
		jdbc.updateGetAutoGenerated("insert into test_generatedkeys(name,dt) values(?,?)", Array(idColumn), "java", (now.plusSeconds(2))).rowsAffected should be(1)
