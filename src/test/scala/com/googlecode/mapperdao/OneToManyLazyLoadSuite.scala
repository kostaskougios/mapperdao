...
class OneToManyLazyLoadSuite extends FunSuite with ShouldMatchers {
	...
	object CarEntity extends Entity[SurrogateIntId, Car] {
		val id = key("id") autogenerated (_.id)
		val model = column("model") to (_.model)

		def constructor(implicit m) = new Car(model) with SurrogateIntId {
	...
	object PersonEntity extends Entity[SurrogateIntId, Person] {
		...
		def constructor(implicit m) = new Person(name, owns, cars) with SurrogateIntId {
