...
object OneToManyAutoGeneratedSpec {
	...
	object JobPositionEntity extends Entity[SurrogateIntId, JobPosition] {
		...
		def constructor(implicit m) = new JobPosition(name, rank) with SurrogateIntId {
	...
	object PersonEntity extends Entity[SurrogateIntId, Person] {
		...
		def constructor(implicit m) = new Person(name, surname, owns, age, m(jobPositions).toList.sortWith(_.name < _.name)) with SurrogateIntId {
