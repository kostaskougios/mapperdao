...
object OneToManyAutoGeneratedSpec {
	...
	object JobPositionEntity extends Entity[IntId, JobPosition] {
		...
		def constructor(implicit m) = new JobPosition(name, rank)  with IntId {
	...
	object PersonEntity extends Entity[IntId, Person] {
		...
		def constructor(implicit m) = new Person(name, surname, owns, age, m(jobPositions).toList.sortWith(_.name < _.name))  with IntId {
