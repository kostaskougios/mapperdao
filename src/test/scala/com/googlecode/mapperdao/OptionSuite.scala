...
class OptionSuite extends FunSuite with ShouldMatchers {
	...
	object CategoryEntity extends Entity[SurrogateIntId, Category] {
		...
		def constructor(implicit m) =
			new Category(name, parent, linked) with SurrogateIntId {
				val id: Int = CategoryEntity.id
			}
	}

	object DogEntity extends Entity[SurrogateIntId, Dog] {
		val id = key("id") autogenerated (_.id)
		val name = column("name") option (_.name)

		def constructor(implicit m) = new Dog(name) with SurrogateIntId {
