...
class TypesSuite extends FunSuite with ShouldMatchers {
	...
	object DatesEntity extends Entity[CustomIntId, Dates] {
		val id = key("id") to (_.id)
		val localDate = column("localDate") to (_.localDate)
		val time = column("time") to (_.time)

		def constructor(implicit m) = new Dates(id, localDate, time) with CustomIntId
	}

	case class ODates(id: Int, localDate: Option[LocalDate])
	object ODatesEntity extends Entity[CustomIntId, ODates]("Dates") {
		val id = key("id") to (_.id)
		val localDate = column("localDate") option (_.localDate)

		def constructor(implicit m) = new ODates(id, localDate) with CustomIntId
	}
	...
	object BDEntity extends Entity[CustomIntId, BD] {
		val id = key("id") to (_.id)
		...
		def constructor(implicit m) = new BD(id, big, bool, nvarchar, text) with CustomIntId
	}
	...
	object OBDEntity extends Entity[CustomIntId, OBD] {
		val id = key("id") to (_.id)
