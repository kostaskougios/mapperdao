...
object ManyToManyMutableAutoGeneratedSpec {
	...
	object ProductEntity extends Entity[IntId, Product] {
		...
		def constructor(implicit m) = new Product(name, m.mutableHashSet(attributes)) with IntId  {
	...
	object AttributeEntity extends Entity[IntId, Attribute] {
		...
		def constructor(implicit m) = new Attribute(name, value, m.mutableHashSet(products))  with IntId {
