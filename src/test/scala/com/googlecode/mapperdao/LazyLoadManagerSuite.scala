...
class LazyLoadManagerSuite extends FunSuite with ShouldMatchers {
	...
	object HouseEntity extends Entity[LongId, House] {
		...
		def constructor(implicit m) = new House(address)  with LongId {
			val id: Long = HouseEntity.id
		}
	}

	object PersonEntity extends Entity[LongId, Person] {
		val id = key("id") autogenerated (_.id)
		...
		def constructor(implicit m) = new Person(name, owns, nearby, m(traversable).toList, array)  with LongId {
