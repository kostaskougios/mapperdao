...
object OneToManySimpleTypesSpecString {
	...
	object ProductEntity extends Entity[SurrogateIntId, Product] {
		...
		def constructor(implicit m: ValuesMap) = new Product(name, tags) with SurrogateIntId {
...
object OneToManySimpleTypesSpecInt {
	...
	object ProductEntityI extends Entity[SurrogateIntId, ProductI] {
		val id = key("id") sequence (Setup.database match {
			case "oracle" => Some("ProductSeq")
			case _ => None
		}) autogenerated (_.id)
		...
		def constructor(implicit m) = new ProductI(name, tags) with SurrogateIntId {
