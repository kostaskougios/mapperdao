...
class ManyToOneAndOneToManyCyclicSuite extends FunSuite with Matchers
{

	import com.googlecode.mapperdao.ManyToOneAndOneToManyCyclicSuite._
	...
	if (Setup.database != "derby") {
		test("update id of one-to-many") {
			...
			mapperDao.select(CompanyEntity, 5).get should be(Company(5, "Coders Ltd", List(Person(10, "Coder1", Company(5, "Coders Ltd", List())))))
