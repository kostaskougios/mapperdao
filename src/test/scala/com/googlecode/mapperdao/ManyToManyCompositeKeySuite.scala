...
class ManyToManyCompositeKeySuite extends FunSuite with ShouldMatchers {
	...
	object UserEntity extends Entity[IntAutoAndStringId, User](if (database == "oracle") "UserX" else "User") {
		...
		def constructor(implicit m) = new User(reference, name, accounts) with IntAutoAndStringId {
			val id: Int = m(UserEntity.id)
		}
	}

	object AccountEntity extends Entity[IntAutoAndLongId, Account] {
		val id = key("id") sequence (
			if (database == "oracle") Some("account_seq") else None
		) autogenerated (_.id)
		val serial = key("serial") to (_.serial)
		val name = column("name") to (_.name)
		def constructor(implicit m) = new Account(serial, name) with IntAutoAndLongId {
