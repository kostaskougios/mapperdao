...
class ManyToManyCompositeKeySuite extends FunSuite with ShouldMatchers {
	...
	if (database != "h2") {
		...
		test("insert, select and delete") {
			...
			val selected = mapperDao.select(UserEntity, inserted.id).get
			selected should be === inserted

			mapperDao.delete(UserEntity, selected)

			mapperDao.select(UserEntity, inserted.id) should be(None)
		}
		...
		test("update, remove") {
			...
			mapperDao.select(UserEntity, updated.id).get should be === updated
		}
	...
	object UserEntity extends Entity[(Int, String), User](if (database == "oracle") "UserX" else "User") {
		...
		def constructor(implicit m) = new User(reference, name, accounts) with SurrogateIntAndNaturalStringId {
			val id: (Int, String) = (UserEntity.id, UserEntity.reference)
		}
	}
	...
	object AccountEntity extends Entity[(Int, Long), Account] {
		...
		def constructor(implicit m) = new Account(serial, name) with SurrogateIntAndNaturalLongId {
			val id: (Int, Long) = (AccountEntity.id, AccountEntity.serial)
		}
	}
