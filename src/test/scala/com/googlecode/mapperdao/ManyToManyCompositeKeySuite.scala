...
class ManyToManyCompositeKeySuite extends FunSuite with ShouldMatchers {
	...
	object UserEntity extends Entity[SurrogateIntAndNaturalStringId, User](if (database == "oracle") "UserX" else "User") {
	...
	object AccountEntity extends Entity[SurrogateIntAndNaturalLongId, Account] {
		val id = key("id") sequence (
